...
/* Construct the AForm with by default the maximum grade which is 1 */
...
AForm::~AForm() { std::cout << this->_name << " AForm destroyed\n"; }

AForm::AForm(const std::string& name, const unsigned int gradeToSign, const unsigned int gradeToExecute):_name(name), _signed(false), _gradeToSign(gradeToSign), _gradeToExecute(gradeToExecute)
{
	if ((gradeToSign < 1) || (gradeToExecute < 1))
		throw GradeTooHighException();	
	if ((gradeToSign > 150) || (gradeToExecute > 150))
		throw GradeTooLowException();
	std::cout << this->_name << " AForm created using perso constructor with grade: " << this->_gradeToSign << " and " << this->_gradeToExecute << std::endl;
}
...
bool AForm::getSigned(AForm *self) 
{
...
const char* AForm::GradeTooLowException::log() const
{
